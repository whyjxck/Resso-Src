...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
        ...
        if (!data.length) {
            return message.reply({ embeds: [new EmbedBuilder().setColor(client.embedColor).setAuthor({ name: `| Please create a playlist before using this command.`, iconURL:message.author.displayAvatarURL()})] })
        }
        if (!args[0]) {
