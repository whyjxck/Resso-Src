...
module.exports = {
    ...
    if(!player) return await interaction.editReply({
        ...
        if(!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new EmbedBuilder().setColor(client.embedColor).setDescription(`Now Playing [${player.queue.current.title}](${client.config.links.support}) • \`[ ${convertTime(player.position)} / ${convertTime(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`);
