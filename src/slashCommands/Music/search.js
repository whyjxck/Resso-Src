        ...
        switch (s.loadType) {
            case "TRACK_LOADED":
                ...
                const embed = new EmbedBuilder()
                    .setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[0].title}](${client.config.links.support}) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)
            ...
            case "SEARCH_RESULT":
                ...
                const results = s.tracks.slice(0, 5).map(x => `• ${index++} | [${x.title}](${client.config.links.support}) \`${convertTime(x.duration)}\``)
                    .join("\n");
                ...
                collector.on("collect", async (b) => {
                    ...
                    if (b.customId === "s_one") {
                        ...
                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[0].title}](${client.config.links.support}) \`${convertTime(s.tracks[0].duration, true)}\` • ${interaction.member.user}`)] })
                    } else if (b.customId === "s_two") {
                        player.queue.add(s.tracks[1]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[1].title}](${client.config.links.support}) \`${convertTime(s.tracks[1].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_three") {
                        player.queue.add(s.tracks[2]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[2].title}](${client.config.links.support}) \`${convertTime(s.tracks[2].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_four") {
                        player.queue.add(s.tracks[3]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[3].title}](${client.config.links.support}) \`${convertTime(s.tracks[3].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_five") {
                        player.queue.add(s.tracks[4]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Added to queue** - [${s.tracks[4].title}](${client.config.links.support}) \`${convertTime(s.tracks[4].duration, true)}\` • ${s.tracks[4].requester}`)] })
